...
save_point = os.path.join(conf.save_dir, '{}-{}.tar'.format('vgrel', 0))

if (os.path.exists(save_point)):
	print ('Previous checkpoint exists in same dir, picking that instead.')
	conf.ckpt = save_point

ckpt = torch.load(conf.ckpt)
if conf.ckpt.split('-')[-2].split('/')[-1] == 'vgrel':
	print("Loading EVERYTHING")
	start_epoch = ckpt['epoch']
	bestmAp = ckpt['best_valmAp']
...
for epoch in range(start_epoch + 1, start_epoch + 1 + conf.num_epochs):
	...
	mAp = val_epoch()
	if (conf.save_dir is not None) and (mAp > bestmAp):
